...
@RestController
@RequestMapping("/api/manual-mining/director")
@RequiredArgsConstructor
@Slf4j
@Tag(name = "Manual Entry director", description = "Manual Entry endpoints are placed here for director")
@SecurityRequirement(name = "bearerAuth")
public class ManualMiningEntryDirectorController {
    ...
    public ResponseEntity<SuccessResponse<List<ManualMiningEntryResponseDTO>>> assignedToGHDChief(
        ...
        return ResponseEntity.ok(
                manualMiningEntryService.getAssignedToDirector(userId, pageable, search)
        );
    }
    ...
    public ResponseEntity<SuccessResponse<ManualMiningEntryResponseDTO>> reviewApplication(
        ...
        Long userId = userContext.getCurrentUserId();
        ManualMiningEntryResponseDTO response = manualMiningEntryService.reviewApplicationDirector(request, userId);
