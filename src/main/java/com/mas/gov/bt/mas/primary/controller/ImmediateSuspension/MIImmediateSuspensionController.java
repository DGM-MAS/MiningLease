...
/**
 * REST Controller for Immediate Suspension management.
 */
@RestController
@RequestMapping("/api/immediate-suspension/mi")
@RequiredArgsConstructor
@Slf4j
@Tag(name = "Immediate Suspension for MI ", description = "Immediate Suspension MI endpoints are placed here")
@SecurityRequirement(name = "bearerAuth")
public class MIImmediateSuspensionController {
    ...
    public ResponseEntity<SuccessResponse<List<ImmediateSuspensionApplicationResponse>>> assignedToMI(
        ...
        return ResponseEntity.ok(immediateSuspensionService.getAssignedToMI(userId, pageable, search)
        );
    }
    ...
    public ResponseEntity<SuccessResponse<ImmediateSuspensionApplicationResponse>> reviewApplication(
        ...
        Long userId = userContext.getCurrentUserId();
        ImmediateSuspensionApplicationResponse response = immediateSuspensionService.reviewApplicationMI(request, userId);
